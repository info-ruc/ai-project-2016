...
def load_data_and_labels():
    ...
    examples_0 = list(open("./data/word/0.txt", "r").readlines())
    examples_0 = [s.strip() for s in examples_0]

    examples_1 = list(open("./data/word/1.txt", "r").readlines())
    examples_1 = [s.strip() for s in examples_1]

    examples_2 = list(open("./data/word/2.txt", "r").readlines())
    examples_2 = [s.strip() for s in examples_2]

    examples_3 = list(open("./data/word/3.txt", "r").readlines())
    examples_3 = [s.strip() for s in examples_3]

    examples_4 = list(open("./data/word/4.txt", "r").readlines())
    examples_4 = [s.strip() for s in examples_4]

    examples_5 = list(open("./data/word/5.txt", "r").readlines())
    examples_5 = [s.strip() for s in examples_5]

    examples_6 = list(open("./data/word/6.txt", "r").readlines())
...
def load_doc2vec(fname, vocab):
    ...
    for word in model.vocab:
        if word in vocab:
            word_vecs[word] = np.array(model[word], dtype='float32') 
    print "load done!"
    print len(word_vecs)
    return word_vecs 
...
def load():
    ...
    w2v = load_doc2vec('./data/vec/doc2vec',vocab)
    tokenizer = Tokenizer()
